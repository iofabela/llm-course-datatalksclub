...
<summary>  Explanation | Docker-network</summary>

This is a short tutorial snippet for setting up PostgreSQL in Docker, with networking so it can later talk to Grafana. Let me break it down piece by piece.
...
This creates a custom Docker network called `monitoring`. By default, containers on Docker can't easily reach each other by name — they need to be on the same user-defined network to do that. Here, the plan is to have both a PostgreSQL container and a Grafana container (for monitoring/dashboards) join this `monitoring` network so Grafana can connect to Postgres using its container name as a hostname (e.g., `course-assistant-pg`) instead of an IP address.

## 2. Running the PostgreSQL container
...
| `-it` | Interactive mode with a pseudo-TTY attached — you'll see Postgres's logs directly in your terminal, and `Ctrl+C` will stop it |
| `--name course-assistant-pg` | Names the container `course-assistant-pg` (used for reference, and as the hostname other containers on the same network can use to reach it) |
| `--network monitoring` | Attaches this container to the `monitoring` network created above |
| `-e POSTGRES_USER=user` | Environment variable: creates a database user called `user` |
| `-e POSTGRES_PASSWORD=password` | Environment variable: sets that user's password |
| `-e POSTGRES_DB=course_assistant` | Environment variable: creates a database named `course_assistant` on startup |
| `-p 5432:5432` | Maps port 5432 on your host machine to port 5432 in the container (Postgres's default port), so you can also connect from your host machine, e.g. via `psql` or a GUI tool |
| `-v pgdata:/var/lib/postgresql/data` | Mounts a named Docker volume called `pgdata` to Postgres's data directory. This means your database data survives even if you remove and recreate the container — without this, all data would be lost when the container is deleted |
| `postgres:17` | The Docker image to use — official Postgres image, version 17 |

## Net effect

After running these two commands, you'll have a PostgreSQL 17 instance:
- accessible from your host machine at `localhost:5432`
- accessible from other containers on the `monitoring` network at `course-assistant-pg:5432`
- with a database `course_assistant`, user `user`, password `password`
- with persistent storage, so data isn't wiped if the container restarts or is recreated

A couple of practical notes: since `-it` keeps the container attached to your terminal, you'd typically want to either run this in a separate terminal tab, or swap `-it` for `-d` (detached mode) if you want it running in the background. Also, `user`/`password` are fine for local dev but shouldn't be used as-is anywhere production-facing.
